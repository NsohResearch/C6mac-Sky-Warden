    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-background via-background to-secondary/30 p-4 sm:p-6">
      ...
      <div className="fixed top-4 right-4 z-40 relative">
        <button
          ...
          className="flex items-center gap-1.5 h-8 px-2.5 rounded-md bg-card border border-border text-xs font-medium text-muted-foreground hover:text-foreground hover:border-accent/40 transition-colors"
        >
          <Globe className="w-3.5 h-3.5" />
          {languages.find((l) => l.code === lang)?.flag}
        </button>
        {langOpen && (
          <div className="absolute top-full right-0 mt-1 bg-card border border-border rounded-lg shadow-lg py-1 min-w-[140px] animate-reveal-up">
            {languages.map((l) => (
              <button
                key={l.code}
